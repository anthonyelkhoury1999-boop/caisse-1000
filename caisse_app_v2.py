...
for i, k in enumerate(ORDER):
    with (c1 if i % 2 == 0 else c2):
        open_counts[k] = st.number_input(k, min_value=0, step=1, value=0)
...
for i, k in enumerate(ORDER):
    with (d1 if i % 2 == 0 else d2):
        in_counts[k] = st.number_input(f"{k} (IN)", min_value=0, step=1, value=0)
...
for i, k in enumerate(ORDER):
    with (w1 if i % 2 == 0 else w2):
        out_counts[k] = st.number_input(f"{k} — quantité à retirer", min_value=0, step=1, value=0)
